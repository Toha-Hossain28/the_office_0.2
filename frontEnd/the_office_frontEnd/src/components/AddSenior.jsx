...
import { createSenior,getSeniorById,deleteSenior, updateSenior } from "../services/SeniorService";
...
const SeniorComponent = () => {
    ...
    useEffect(() => {
        if(id) {
            getSeniorById(id).then((response) => {
                const senior = response.data;
                setFirstName(senior.firstName);
                setLastName(senior.lastName);
                setEmail(senior.email);
                setPhone(senior.phone);
                setSalary(senior.salary);
                setDepartment(senior.department);
                setEmployeeType(senior.employeeType);
                setMainSkill(senior.mainSkill);
            }).catch((error) => {
                console.error(error);
            });
        }
    ...
    function saveOrUpdateSenior(event) {
        ...
        if (validateForm()) {
            ...
            if (id) {
                updateSenior(id, senior).then((response) => {
                ...
                createSenior(senior).then((response) => {
                    console.log(response.data);
                    navigator('/senior');
                }).catch((error) => {
                    console.error(error);
    ...
    function pageTitle() {
        if(id) {
            return <h2 className="text-center mb-4">Update Senior</h2>;
        } else {
            return <h2 className="text-center mb-4">Add Senior</h2>;
        }
    }
                    ...
                    <div className="row mb-3">
                        ...
                        <div className="col">
                            ...
                            <div className="text-center">
                                <button type="submit" className="btn btn-success" onClick={saveOrUpdateSenior}>Save</button>
                            </div>
                        </form>
                    </div>
                </div>
