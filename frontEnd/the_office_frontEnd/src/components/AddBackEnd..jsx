...
import { createBackEnd, getBackEndById, updateBackEnd } from "../services/BackEndService";
...
const BackEndComponent = () => {
    ...
    useEffect(() => {
        if (id) {
            getBackEndById(id).then((response) => {
                const backend = response.data;
                console.log("Backend data: ", backend); // Add this line
                setFirstName(backend.firstName);
                setLastName(backend.lastName);
                setEmail(backend.email);
                setPhone(backend.phone);
                setSalary(backend.salary);
                setDepartment(backend.department);
                setEmployeeType(backend.employeeType);
                setMainSkill(backend.mainSkill);
            }).catch((error) => {
                console.error(error);
            });
        }
    ...
    function saveOrUpdateBackEnd(event) {
        ...
        if (validateForm()) {
            ...
            if (id) {
                updateBackEnd(id, backend).then((response) => {
                ...
                createBackEnd(backend).then((response) => {
                    console.log(response.data);
                    navigator('/backend');
                }).catch((error) => {
                    console.error(error);
    ...
    function pageTitle() {
        if (id) {
            return <h2 className="text-center mb-4">Update BackEnd</h2>;
        } else {
            return <h2 className="text-center mb-4">Add BackEnd</h2>;
        }
    }
                    ...
                    <div className="card-body">
                        <form>
                            ...
                            <div className="text-center">
                                <button type="submit" className="btn btn-success" onClick={saveOrUpdateBackEnd}>Save</button>
                            </div>
                        </form>
                    </div>
                </div>
