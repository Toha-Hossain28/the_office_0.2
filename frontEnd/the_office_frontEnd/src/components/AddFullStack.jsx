...
import { createFullStack,getFullStackById,updateFullStack } from "../services/FullStackService";
...
const FullStackComponent = () => {
    ...
    useEffect(() => {
        if(id) {
            getFullStackById(id).then((response) => {
                const fullstack = response.data;
                setFirstName(fullstack.firstName);
                setLastName(fullstack.lastName);
                setEmail(fullstack.email);
                setPhone(fullstack.phone);
                setSalary(fullstack.salary);
                setDepartment(fullstack.department);
                setEmployeeType(fullstack.employeeType);
                setMainSkill(fullstack.mainSkill);
            }).catch((error) => {
                console.error(error);
            });
        }
    ...
    function saveOrUpdateFullStack(event) {
        ...
        if (validateForm()) {
            ...
            if (id) {
                updateFullStack(id, fullstack).then((response) => {
                ...
                createFullStack(fullstack).then((response) => {
                    console.log(response.data);
                    navigator('/fullstack');
                }).catch((error) => {
                    console.error(error);
    ...
    function pageTitle() {
        if(id) {
            return <h2 className="text-center mb-4">Update FullStack</h2>;
        } else {
            return <h2 className="text-center mb-4">Add FullStack</h2>;
        }
    }
                    ...
                    <div className="row mb-3">
                        ...
                        <div className="col">
                            ...
                            <div className="text-center">
                                <button type="submit" className="btn btn-success" onClick={saveOrUpdateFullStack}>Save</button>
                            </div>
                        </form>
                    </div>
                </div>
