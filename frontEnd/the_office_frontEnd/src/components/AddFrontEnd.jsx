...
import { createFrontEnd,getFrontEndById,updateFrontEnd } from "../services/FrontEndService";
...
const FrontEndComponent = () => {
    ...
    useEffect(() => {
        if(id) {
            getFrontEndById(id).then((response) => {
                const frontend = response.data;
                setFirstName(frontend.firstName);
                setLastName(frontend.lastName);
                setEmail(frontend.email);
                setPhone(frontend.phone);
                setSalary(frontend.salary);
                setDepartment(frontend.department);
                setEmployeeType(frontend.employeeType);
                setMainSkill(frontend.mainSkill);
            }).catch((error) => {
                console.error(error);
            });
        }
    ...
    function saveOrUpdateFrontEnd(event) {
        ...
        if (validateForm()) {
            ...
            if (id) {
                updateFrontEnd(id, frontend).then((response) => {
                ...
                createFrontEnd(frontend).then((response) => {
                    console.log(response.data);
                    navigator('/frontend');
                }).catch((error) => {
                    console.error(error);
    ...
    function pageTitle() {
        if(id) {
            return <h2 className="text-center mb-4">Update FrontEnd</h2>;
        } else {
            return <h2 className="text-center mb-4">Add FrontEnd</h2>;
        }
    }
                    ...
                    <div className="row mb-3">
                        ...
                        <div className="col">
                            ...
                            <div className="text-center">
                                <button type="submit" className="btn btn-success" onClick={saveOrUpdateFrontEnd}>Save</button>
                            </div>
                        </form>
                    </div>
                </div>
